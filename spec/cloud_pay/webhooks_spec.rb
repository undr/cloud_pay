...
RSpec.describe CloudPay::Webhooks do
  describe 'HMAC validation' do
    let(:webhooks) { CloudPay::Webhooks.new(:test) }
    ...
    context 'with data_valid?' do
      it 'returns true on valid hmac' do
        expect(webhooks.data_valid?(data, valid_hmac)).to be true
      end

      it 'returns false on invalid hmac' do
        expect(webhooks.data_valid?(data, invalid_hmac)).to be false
      end
    end

    context 'with validate_data!' do
      it 'returns true on valid hmac' do
        expect(webhooks.validate_data!(data, valid_hmac)).to be nil
      end

      it 'raises a HMACError on invalid hmac' do
        expect { webhooks.validate_data!(data, invalid_hmac) }.to raise_error(CloudPay::HMACError)
      end
    end
  end
end
